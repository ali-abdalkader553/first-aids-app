...
class DataStateOfExplanations extends StatefulWidget {
  ...
  const DataStateOfExplanations({Key? key, required this.firstaidid}) : super(key: key);
...
class _DataStateOfExplanations extends State<DataStateOfExplanations > {
  ...
  getData() async{
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection("first-aids").doc(widget.firstaidid).collection("explanation").get();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          Navigator.of(context).push(MaterialPageRoute(builder: (context)=> Add_Explanation(docid:widget.firstaidid)));
        },
              ...
              onLongPress: (){
                AwesomeDialog(
                  ...
                  btnCancelOnPress: () async{
                    await FirebaseFirestore.instance.collection("first-aids").doc(firstaidsdata[i].id).delete();
                    Navigator.push(
                        context,
                  ...
                  btnOkOnPress: (){
                    Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => Edit_First_Aids(docid: '', oldname: firstaidsdata[i]['name']))
                    );
